...
dsearch2={'object':rfeature[0],'key':2,'valueset':[20,5,5.4,5.8,6,]}#[6,5.4,5,4.8,4.6,4.4,4.2]#15, 7,6,5,4.5,4,3.5,3#,10,8,7,6,5#15,12,10, 8,7,15,12,10, 8,7,6,

#dsearch3={'object':[rfeature[0],sfeature[0]],'key':[3,3],'valueset':[[0.025,0.025],[0.05,0.05],[0.1,0.1],[0.25,0.25],[0.5,0.5]]}
dsearch4={'object':par,'key':'uniform','valueset':[8,6,5,4,3]}
...
model1={'scorers':[scaledsp1,ref1],'bmtype':bmtype,'searches':[search1,search2], 'runsperscorer':ni,
    'dsearches':[dsearch2,dsearch4,dsearch7,dsearch8],'sml':sml,'cvk':4,'repeat':1,'fold':3}#,dsearch2,dsearch5,dsearch6,dsearch7 #,'testperc':0.33
#dsearch1,dsearch2,dsearch6,dsearch7
if 0:
                so=scorer(model=convert2old(model1))
                print so.assess_ideal()
                print so.assess_worst()
                print so.assess_sascore()  
                pdb.set_trace()
